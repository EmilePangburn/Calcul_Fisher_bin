...
#pour gagner du temps de calcul, l'idée est de calculer toutes les dérivés des c_l en parallèle puis de les réunir ensuite
#calcul les éléments de la matrice de Fisher à partir des fichiers des dérivées
def Fisher_ij_bin_EE(i,j,sP,theta_FWHM):
    F_ij=0
    #changer le nom pour avoir les bons fichiers
    deriv_i = np.loadtxt('deriv_'+str(i)+'.txt')
    deriv_j = np.loadtxt('deriv_'+str(i)+'.txt')
    #changer le nom pour avoir les bons fichiers
    c_l = np.loadtxt('c_l.txt')
    for l in range(2,200):
        mat_cl = totCL[l,1]+noise(sP,l,theta_FWHM)
        matder1 = deriv_i[2][l]
        matder2 = deriv_j[2][l]
...
#calcul la matrice de Fisher à partir des fichiers des dérivées
def Full_Fisher_bin(sT,sP,theta_FWHM,bins):
    f_sky = 1
    liste = []
    for i in range(1,97):
    ...
    for i in range(1,97):
        amplitude["a"+str(i)]=i
    parameter = liste
    Fisher = np.zeros((96,96))
    for i in range(96):
        for j in range(i,96):
            Fisher[i,j] = Fisher_ij_bin_EE(i,j,sP,theta_FWHM)
            print(i,j)
...
#fraction d'ionisation initiale
